...
"""
    A simple fixed max size ring buffer implementation for storing timestamps.
    Rate limiting is computed based on task completions not task admissions.
    The core philosophy is safety/correctness over raw throughput.
"""
...
class RateLimiter:
    ...
    def __init__(self, rate: int, per_ns: int) -> None:
        self.ring_buffer: RingBuffer = RingBuffer(rate)
        self.rate: int = rate
